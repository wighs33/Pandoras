...
#include "GA/GA_FinishAttack_Sword.h"
...
void UGA_FinishAttack_Sword::ActivateAbility(const FGameplayAbilitySpecHandle Handle,
...
bool UGA_FinishAttack_Sword::CheckConditions()
{
    AActor* Avatar = GetAvatarActorFromActorInfo();

    // --- 오너의 스태미너가 25보다 큰 지 체크 ---------------------------------------
    const float Stamina = Avatar ? ICharacterInterface::Execute_GetStamina(Avatar) : 0.f;
...
bool UGA_FinishAttack_Sword::CheckVictim() const
{
...
void UGA_FinishAttack_Sword::ShowTip()
{
    AActor* Avatar = GetAvatarActorFromActorInfo();
    if (!Avatar) return;
...
void UGA_FinishAttack_Sword::RunNormal()
{
    ...
    if (Pair.Attacker)
    {
        ...
        if (Task)
        {
            Task->OnInterrupted.AddDynamic(this, &UGA_FinishAttack_Sword::OnMontageFinish);
    ...
    StabInTask->EventReceived.AddDynamic(
        this,
        &UGA_FinishAttack_Sword::ShowBlood
    );
    ...
    StabOutTask->EventReceived.AddDynamic(
        this,
        &UGA_FinishAttack_Sword::Die
    );

    StabOutTask->ReadyForActivation();
}

void UGA_FinishAttack_Sword::ShowBlood(FGameplayEventData Payload)
{
    // --- 오너의 무기에 출혈 이펙트 재생 -------------------------------------------
    AActor* Avatar = GetAvatarActorFromActorInfo();
...
void UGA_FinishAttack_Sword::Die(FGameplayEventData Payload)
{
    ShowBlood(Payload);
...
void UGA_FinishAttack_Sword::RunFromEvent(const FGameplayEventData* EventData)
{
    // --- 피해상대 저장 --------------------------------------------------------
...
void UGA_FinishAttack_Sword::OnMontageFinish()
{
    K2_EndAbility();
}